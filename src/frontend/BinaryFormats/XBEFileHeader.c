...
void
SgAsmPEFileHeader::ctor() //Will most likely want to make one for XBE as well.
{
...
SgAsmPEFileHeader::is_PE(SgAsmGenericFile *file)
{
    ...
    try {
        ...
        if ('P'!=pe_magic[0] || 'E'!=pe_magic[1] || '\0'!=pe_magic[2] || '\0'!=pe_magic[3]) // Modify this in order to direct it to recognize the XBEH header instead.
            throw 1;
    } catch (...) {
...
SgAsmPEFileHeader::parse()
{
    ...
    if (fh.e_magic[0]!='P' || fh.e_magic[1]!='E' || fh.e_magic[2]!='\0' || fh.e_magic[3]!='\0') //Once again, modify this to accept the XBEH magic number
        throw FormatError("Bad PE magic number");

    /* Decode COFF file header */
    p_e_cpu_type           = ByteOrder::le_to_host(fh.e_cpu_type); //This is incredibly similar, however there are very likely things that need to be added here.
    ...
    if (4==p_exec_format->get_word_size()) { //Add additional header values to decode specifically for XBE
        ...
        entry_rva              = ByteOrder::le_to_host(oh32.e_entrypoint_rva); // need to modify this to do the xor math needed to decode the entry point
